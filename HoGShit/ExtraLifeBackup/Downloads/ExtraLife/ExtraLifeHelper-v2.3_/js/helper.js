...
{
    testName = "John Smith";
    testAmount = (Math.random() < 0.25 ? Math.random() * 1.5 : Math.random() * 128);
    ...
    testCreatedOn = "2019-10-05T01:09:59.97+0000";
...
function initSound()
{
    ...
    donationSounds.forEach(function(path) {
			soundObjects.set(path, new Audio("audio/" + path));
		});
...
function showNewDonor(donorName, donorAmount, donorMessage, donorAvatar, donorCreatedOn)
{
    ...
    logoGroup.visible = false;
		...
		let speechDelay = 5;
		let soundObject;
		let targetSound = donationSoundPicker(donorName, donorAmount, donorMessage, donorAvatar, donorCreatedOn).trim();
		//console.log("picked \"" + targetSound + "\"");
		if (soundObjects.has(targetSound)) {
			soundObject = soundObjects.get(targetSound);
			if (soundObject) {
				if (soundObject.duration > 0) {
					speechDelay = soundObject.duration;
				}
				soundObject.load();
				console.log("playing \"" + targetSound + "\"");
				soundObject.play();
			}
		}
